 ...
 */
@SuppressLint("NewApi")
public class GamePlay extends ActionBarActivity implements View.OnClickListener, DialogInterface.OnClickListener, View.OnTouchListener {
	...
	public boolean onTouch(View view, MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (tentative != null) {
				...
				if (type == GameType.COMPUTER || type == GameType.REMOTE) runOpponentMove(tentative);
			} else {
		...
		if (clicked == buttonAIMove) {
			...
			for (int i = 0; i < times; i++) {
				...
				if (toUndo.moved instanceof Pawn || toUndo.moved instanceof Enpassant){
					if (toUndo.moved.getOwner().equals("White") && toUndo.from.getI() == 6) toUndo.moved.reset();
					else if (toUndo.moved.getOwner().equals("Black") && toUndo.from.getI() == 1) toUndo.moved.reset();
				}
