...
public class Chess implements ChessRules, Serializable {
  ...
  public void play(Board board, Player playerWhite, Player playerBlack) {
    while (canMove) {
      if (turns % 2 == 0) {
      ...
      if (turns % 2 == 0) {
        if (movePiece(board.getPiece(new Location(-1, -1)), board, new Location(-1, -1), playerWhite)) {
          turns++;
        } else {
        ...
        if (movePiece(board.getPiece(new Location(-1, -1)), board, new Location(-1, -1), playerBlack)) {
          turns++;
        } else {
          System.out.println("Illegal move, try again");
          continue;
      ...
      if (blackCheck()) {
        if (checkMate(board, playerBlack)) {
          System.out.println("Checkmate\n\nWhite wins\n");
          break;
        }
        System.out.println("Check");
      } else if (whiteCheck()) {
        if (checkMate(board, playerWhite)) {
          System.out.println("Checkmate\n\nBlack wins\n");
          break;
        }
        System.out.println("Check\n");
        ...
        if (turns % 2 == 0) {
          if (stalemate(board, playerWhite)) {
            System.out.println("\nStalemate\n\nDraw");
            break;
          }
        } else {
          if (stalemate(board, playerBlack)) {
            System.out.println("\nStalemate\n\nDraw");
            break;
          }
        }
  ...
  public void calculateMoves(Board board) {
    ...
    for (int i = 0; i < 8; i++) {
      for (int j = 0; j < 8; j++) {
        ...
        //iterate through move-set unit vectors
        for (int index = 0; index < vectors.length; index++) {
          ...
          for (int offset = 0; offset < moveDist; offset++) {
            ...
            if ((iOff >= 0 && iOff <= 7) && (jOff >= 0 && jOff <= 7)) {
              if (board.isEmpty(iOff, jOff)) {
              ...
              } else if (!(piece.getOwner().equals(board.getPiece(
                  new Location(iOff, jOff)).getOwner()))) {
                if (piece instanceof Pawn) {
                  break;
                }
                piece.addValidMove(new Location(iOff, jOff));
                if (board.getPiece(new Location(iOff, jOff)) instanceof King
                  ...
                  if (board.getPiece(new Location(iOff, jOff))
                      .getOwner().equals("White")) {
                    whiteCheck = true;
                  } else {
                    blackCheck = true;
                  }
                  ...
                  if (board.isEmpty(iOff, jOff)) {
                    if (offset == 1) {
                      ...
                      if (!board.isEmpty(iOff, jOff + sVectors[index][1])) {
                        if ((piece.getOwner().equals(board.getPiece(
                            new Location(iOff, jOff + sVectors[index][1])).getOwner()))
                            && (board.getPiece(new Location(iOff, jOff + sVectors[index][1])) instanceof Rook)
                            && (board.getPiece(new Location(iOff, jOff + sVectors[index][1])).atStart())) {
                          piece.addValidMove(new Location(iOff, jOff));
                        }
                      }
                      //left
                      if ((iOff >= 0 && iOff <= 7) && ((jOff + 2 * sVectors[index][1]) >= 0
                          && (jOff + 2 * sVectors[index][1]) <= 7)) {
  ...
  public boolean movePiece(Piece piece, Board board, Location toMove, Player player) {
    ...
    if (!board.isEmpty(toMove.getI(), toMove.getJ())
        || board.isEnpassant(toMove.getI(), toMove.getJ())) {
  ...
  public boolean checkMate(Board board, Player player) {
    ...
    outerloop2: //lol jump statements again
    for (int i = 0; i < 8; i++) {//check if available king moves won't put the king in check
      for (int j = 0; j < 8; j++) {
        ...
        if (piece == null) {
          ...
          assert kingInCheck != null;
          if (piece.getOwner().equals(kingInCheck.getOwner())) {
            continue; //if piece is identical, +1 on loop
          }
        }
        ...
        if (pieceMoves.size() == 0) {
          continue; //if no valid moves on enemy piece, +1 on loop
        }
        if (kingMoves.size() == 0) {
          break outerloop2;
        }
        outerloop3: // I swear I don't use jump statements this much!
        for (int x = 0; x < kingMoves.size(); x++) {
          for (int z = 0; z < pieceMoves.size(); z++) {
            if ((pieceMoves.get(z).getI() == kingMoves.get(x).getI())
                && (pieceMoves.get(z).getJ() == kingMoves.get(x).getJ())) {
  ...
  public ArrayList<Location> parseInput(String input) {
    ...
    if (userCommands.size() == 4 || userCommands.size() == 5) {
      for (int i = 0; i < 4; i += 2) {
        if (!((int) userCommands.get(i).charAt(0) - 97 >= 0 && (int) userCommands.get(i).charAt(0) - 97 < 8)) {
          System.err.println("Error: please enter a valid board location, i.e. \"e5\"");
          return null;
        }
      ...
      if (userCommands.size() == 5) {
        if (!userCommands.get(4).equalsIgnoreCase("draw?")) {
          System.err.println("Error: The only accepted command following board locations is \"draw?\"");
          return null;
        } else {
    ...
    System.err.println("Error: Improper input, please use <piece location> <location to move> [\"draw?\"]");
    return null;
  }
